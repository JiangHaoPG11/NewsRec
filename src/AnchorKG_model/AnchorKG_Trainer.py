    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            user_act_probs_steps, user_q_values_steps, user_step_rewards, user_graph, user_graph_relation, user_graph_type = self.subgraph_model(
                user_index, user_clicked_newsindex,
                candidate_newsindex)  # [[bz, d(1-hop) * 1], [bz, d(1-hop) * d(2-hop)], [bz, d(1-hop) * d(2-hop) * d(3-hop)]]

            rea_score, overlap_score, path_num, _, overlap_num = self.model_reasoner(candidate_newsindex, user_index, news_graph, user_graph,
            ...
            rec_score = self.model_recommender(candidate_newsindex, user_index, news_graph, user_graph)
        ...
        with no_grad():
            for data in self.vaild_dataloader:
                ...
                rea_score = self.model_reasoner(
                    candidate_newsindex, user_index, news_graph, user_graph, news_graph_relation, user_graph_relation
                )[0]

                rec_score = self.model_recommender(
                    candidate_newsindex, user_index, news_graph, user_graph
                )
        ...
        for epoch in range(1, self.args.epoch+1):
            ...
            if predict_graph:
                ...
                for data in self.train_dataloader:
                    ...
                    news_graph_nodes.extend(self.subgraph_model.get_subgraph_list(
                        self.subgraph_model(
                            user_index,user_clicked_newsindex,candidate_newsindex
                        )[3],
                        self.subgraph_model(
                    ...
                    if not os.path.exists("./AnchorKG_out"):
                        os.makedirs("./AnchorKG_out")

                    news_graph_file = open("./AnchorKG_out/news_file_" + str(epoch) + ".tsv", 'w', encoding='utf-8')
                    for i in range(self.args.batch_size):
                        news_graph_file.write(candidate_newsindex[i] + '\t' + ' '.join(list(set(news_graph_nodes[i]))) + '\n')
                    user_graph_file = open("./AnchorKG_out/user_file_" + str(epoch) + ".tsv", 'w', encoding='utf-8')
                    for i in range(self.args.batch_size):
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                news_graph_nodes_num, user_graph_nodes_num = self.model_reasoner(
                    candidate_newsindex, user_index, news_graph, user_graph, news_graph_relation, user_graph_relation
                )
