...
class Recommender(torch.nn.Module):
    def __init__(self, args, news_title_embedding, entity_embedding, relation_embedding, news_entity_dict, entity_adj, relation_adj,
        ...
        self.user_extend_1 = nn.Linear(self.args.embedding_size, self.args.attention_dim * self.args.attention_heads)
        self.user_extend_2 = nn.Linear(self.args.attention_dim * self.args.attention_heads,
                                       self.args.attention_dim * self.args.attention_heads)
    ...
    def get_user_news_rep(self, candidate_news_index, user_index):
        ...
        user_rep = torch.tanh(self.user_extend_2(self.elu(self.user_extend_1(user_embedding))))
        score = torch.sum(news_rep * user_rep.unsqueeze(1), dim = -1)
        return score
    ...
    def forward(self, candidate_news, user_index, news_graph, user_graph):
        ...
        # 新闻用户表征
        score = self.get_user_news_rep(candidate_news, user_index)
