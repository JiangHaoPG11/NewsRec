...
def parse_args():
    ...
    parser.add_argument('--news_num', type=int, default=4139, help='标题新闻总数')
...
def metadata_generator(args, path):
    ...
    # 训练集大小
    train_size = int((len(Bound_test) * 0.7) * args.train_ratio)
...
def load_data(args, path):
    # train & vaild & test data generation
    ...
    train_data = Train_Dataset(
        candidate_newsindex_train, user_clicked_newsindex,
    ...
    # construct train dataloader
    vaild_data = Vaild_Dataset(
        candidate_newsindex_vaild, user_clicked_newsindex,
        user_index_vaild, label_vaild, user_type, news_type
    )
    vaild_dataloader = DataLoader(
        vaild_data, batch_size=args.batch_size, shuffle=True, drop_last=True
    )

    # contruct test dataloader
    test_data = Test_Dataset(
        candidate_newsindex_test, user_clicked_newsindex,
