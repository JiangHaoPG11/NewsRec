...
class Trainer():
    def __init__(self, args, NRMS_model, optimizer_NRMS, data):
        self.args = args
        self.NRMS_model = NRMS_model
        self.optimizer_NRMS = optimizer_NRMS
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.NRMS_model(candidate_newsindex, user_clicked_newsindex)
    ...
    def _vaild_epoch(self):
        self.NRMS_model.eval()
        rec_auc_list = []
        with no_grad():
            pbar = tqdm(total=self.vailddata_size)
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.NRMS_model.test(candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_NRMS_model = self.NRMS_model.state_dict()
        filename_NRMS = self.args.checkpoint_dir + ('checkpoint-NRMS-epoch{}.pth'.format(epoch))
        torch.save(state_NRMS_model, filename_NRMS)
    ...
    def test(self):
        ...
        # np.save('NRMS_user_rep.npy', np.array(user_rep_list))
        ...
        # np.save('NRMS_news_rep.npy', np.array(news_rep_list))
        ...
        # plt.legend()
        # plt.savefig("NRMS_news_rep.png")
        # plt.show()
        # plt.close()
        
        # 存储预测结果
        folder_path = '../predict/NRMS/'
        ...
        predict_df.to_csv(f'{folder_path}/NRMS_predict.csv', index = False)
