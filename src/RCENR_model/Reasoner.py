...
class Reasoner(torch.nn.Module):
    def __init__(self, args, entity_embedding, relation_embedding, news_title_embedding, device):
        ...
        self.elu = nn.ELU()
        self.tanh = nn.Tanh()
        self.gru = torch.nn.GRU(self.args.embedding_dim, self.args.embedding_dim)
        self.gru_output_layer1 = nn.Linear(self.args.embedding_dim, self.args.embedding_dim)
        self.gru_output_layer2 = nn.Linear(self.args.embedding_dim, 1)
    ...
    def forward(self, candidate_news, user_index,
        ...
        return predicts, reason_num.reshape(self.args.batch_size, self.args.sample_size), \
               ...
               news_graph_list, user_graph_list, \
               news_graph_nodes_num, user_graph_nodes_num
