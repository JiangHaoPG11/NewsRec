...
class AnchorKG(torch.nn.Module):
    ...
    def __init__(self, args, news_entity_dict, entity_news_dict, user_click_dict,
                 news_category_index, category_news_dict,
                 news_title_embedding, entity_embedding, relation_embedding,
                 entity_adj, relation_adj, device):
        ...
        # embedding
        self.user_embedding = nn.Embedding(self.args.user_num, self.args.embeddign_dim).to(self.device)
        self.category_embedding = nn.Embedding(self.args.category_num, self.args.embeddign_dim).to(self.device)
        self.subcategory_embedding = nn.Embedding(self.args.subcategory_num, self.args.embeddign_dim).to(self.device)
        ...
        self.news_compress_1 = nn.Linear(self.args.title_embedding_dim, self.args.embeddign_dim)
        self.news_compress_2 = nn.Linear(self.args.embeddign_dim, self.args.embeddign_dim)
    ...
    def get_news_reward(self, graph_embedding, user_embedding, action_input, depth):
        nodes_embedding = self.get_nodes_embedding(action_input, depth, mode = 'news')
        reward, graph_embedding = self.get_sim_reward_batch(graph_embedding, user_embedding, nodes_embedding)
        # hit_reward = self.get_hit_rewards(action_input)
        return reward , graph_embedding

    def get_user_reward(self, graph_embedding, news_embedding, action_input, depth):
        nodes_embedding = self.get_nodes_embedding(action_input, depth, mode = 'user')
        reward, graph_embedding = self.get_sim_reward_batch(graph_embedding, news_embedding, nodes_embedding)
        # hit_reward = self.get_hit_rewards(action_input)
        return reward , graph_embedding
    ...
    def get_hit_rewards(self, action_input):

        hit_rewards = []
        hit_rewards_weak = []
        for i in range(action_input.shape[0]):
            hit_rewards.append([])
            hit_rewards_weak.append([])
            for j in range(action_input.shape[1]):
                if int(action_input[i][j].data.cpu().numpy()) in self.entity_news_dict:
                    entity_neibor = set(self.entity_news_dict[int(action_input[i][j].data.cpu().numpy())])
                ...
                if int(action_input[i][j].data.cpu().numpy()) in self.category_news_dict:
                    category_neibor = set(self.category_news_dict[int(action_input[i][j].data.cpu().numpy())])
                    if category_neibor != None:
                        if len(category_neibor) > 0:
                            hit_rewards_weak[-1].append(1.0)
                        else:
                            hit_rewards_weak[-1].append(0.0)
                    else:
                        hit_rewards_weak[-1].append(0.0)
                else:
                    hit_rewards_weak[-1].append(0.0)

        return torch.FloatTensor(hit_rewards).to(self.device) + torch.FloatTensor(hit_rewards_weak).to(self.device)
    ...
    def get_state_input(self, news_embedding, depth, history_entity, history_relation, history_type, mode = None):
        if depth == 0:
            state_embedding = torch.cat([news_embedding,
                                         torch.FloatTensor(np.zeros((news_embedding.shape[0], 100))).to(self.device)], dim=-1)
        else:
            if mode == 'news':
            ...
            state_embedding_news = history_relation_embedding + history_action_embedding #  bz, d(k-hop-1), dim
    ...
    def get_action_embedding(self, action_id, relation_id, type_id, mode = None, hop_num = None):
        ...
        return action_embedding + relation_embedding
    ...
    def forward(self, user_index, user_clicked_news_index, candidate_newsindex):
        ...
        while (depth < self.MAX_DEPTH):
            ...
            step_reward, news_graph_embedding = self.get_news_reward(news_graph_embedding, user_embedding, step_news_graph_nodes, depth)  # bz, d(1-hop)  # bz*5, d(1-hop) * d(2-hop) # bz*5, d(1-hop) * d(2-hop) * d(3-hop)
        ...
        while (depth < self.MAX_DEPTH):
            ...
            step_reward, user_graph_embedding = self.get_user_reward(user_graph_embedding, news_embedding, step_user_graph_nodes, depth)  # bz, d(1-hop)  # bz*5, d(1-hop) * d(2-hop) # bz*5, d(1-hop) * d(2-hop) * d(3-hop)
