...
class Trainer():
    def __init__(self, args, Baseline_model, optimizer_Baseline, data):
        self.args = args
    ...
    def optimize_baseline(self, score, label):
        rec_loss = F.cross_entropy(score.cpu(), torch.argmax(label, dim=1))
    ...
    def _train_epoch(self):
        ...
        for data in self.train_dataloader:
            ...
            score = self.Baseline_model(user_index, user_clicked_newsindex, candidate_newsindex)
            rec_loss = self.optimize_baseline(score, label)
            rec_auc = self.cal_auc(score, label)
    ...
    def _save_checkpoint(self, epoch):
        state_agent = self.Baseline_model.state_dict()
        filename = self.args.checkpoint_dir + ('checkpoint-baseline-epoch{}.pth'.format(epoch))
        torch.save(state_agent, filename)
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                score = self.Baseline_model.test(user_index, user_clicked_newsindex, candidate_newsindex)
        ...
        predict_df.to_csv(f'{folder_path}/Baseline_predict.csv', index = False)
