from model.DKN_model.DKN import DKN
from model.DKN_model.DKN_Trainer import Trainer
import torch

class DKN_Train_Test():
    def __init__(self, args, data, device):
        ...
        DKN_model = DKN(args, entity_embedding, relation_embedding,
        ...
        optimizer_DKN = torch.optim.Adam(DKN_model.parameters(), lr=0.0001)
        self.trainer = Trainer(args, DKN_model, optimizer_DKN, data)
    ...
    def Test_load(self):
        self.DKN_model.load_state_dict(torch.load(self.args.checkpoint_dir + 'checkpoint-' + self.args.mode + '-epochfinal.pth'))
        self.trainer.test()
