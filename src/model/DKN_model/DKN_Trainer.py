...
class Trainer():
    def __init__(self, args, DKN_model, optimizer_DKN, data):
        self.args = args
        self.DKN_model = DKN_model
        self.optimizer_DKN = optimizer_DKN
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.DKN_model(candidate_newsindex, user_clicked_newsindex)
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.DKN_model.test(candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_DKN_model = self.DKN_model.state_dict()
        filename_DKN = self.args.checkpoint_dir + ('checkpoint-DKN-epoch{}.pth'.format(epoch))
        torch.save(state_DKN_model, filename_DKN)
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                score = self.DKN_model.test(candidate_newsindex, user_clicked_newsindex)
        ...
        predict_df.to_csv(f'{folder_path}/DKN_predict.csv', index = False)
