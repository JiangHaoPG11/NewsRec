from model.FIM_model.FIM import FIM
from model.FIM_model.FIM_Trainer import Trainer
import torch

class FIM_Train_Test():
    def __init__(self, args, data, device):
        ...
        FIM_model = FIM(args, entity_embedding, relation_embedding,
        ...
        optimizer_FIM = torch.optim.Adam(FIM_model.parameters(), lr=0.0001)
        self.trainer = Trainer(args, FIM_model, optimizer_FIM, data)
    ...
    def Test_load(self):
        self.FIM_model.load_state_dict(torch.load(self.args.checkpoint_dir + 'checkpoint-' + self.args.mode + '-epochfinal.pth'))
        self.trainer.test()
