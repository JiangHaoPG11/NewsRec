...
class Trainer():
    def __init__(self, args, FIM_model, optimizer_FIM, data):
        self.args = args
        self.FIM_model = FIM_model
        self.optimizer_FIM = optimizer_FIM
        self.save_period = 1
        self.vaild_period = 1
    ...
    def optimize_model(self, rec_score, label):
        ...
        self.optimizer_FIM.step()
        return rec_loss

    def _train_epoch(self, epoch):
        self.FIM_model.train()
        rec_all_loss_list = []
        auc_list = []
        rec_all_loss = 0
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.FIM_model(candidate_newsindex, user_clicked_newsindex)
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.FIM_model.test(candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_FIM_model = self.FIM_model.state_dict()
        filename_FIM = self.args.checkpoint_dir + ('checkpoint-FIM-epoch{}.pth'.format(epoch))
        torch.save(state_FIM_model, filename_FIM)
    ...
    def test(self):
        ...
        predict_df.to_csv(f'{folder_path}/FIM_predict.csv', index = False)
