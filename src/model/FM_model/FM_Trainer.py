...
class Trainer():
    def __init__(self, args, FM_model, optimizer_FM, data):
        self.args = args
        self.FM_model = FM_model
        self.optimizer_FM = optimizer_FM
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.FM_model(user_index, candidate_newsindex)
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.FM_model.test(user_index, candidate_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_FM_model = self.FM_model.state_dict()
        filename_FM = self.args.checkpoint_dir + ('checkpoint-FM-epoch{}.pth'.format(epoch))
        torch.save(state_FM_model, filename_FM)
    ...
    def test(self):
        ...
        predict_df.to_csv(f'{folder_path}/FM_predict.csv', index = False)
        ...
        w_AUC, w_MRR, w_nDCG5, w_nDCG10, w_len, = evaluate_warm_cold_u(pred_label_list, user_type_list, news_type_list, self.label_test, self.bound_test)
