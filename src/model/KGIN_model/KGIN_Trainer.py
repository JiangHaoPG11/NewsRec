...
class Trainer():
    def __init__(self, args, KGIN_model, optimizer_KGIN, data):
        self.args = args
        self.KGIN_model = KGIN_model
        self.optimizer_KGIN = optimizer_KGIN
    ...
    def optimize_MMCCLK(self, loss,):
        self.optimizer_KGIN.zero_grad()
        loss.backward(retain_graph=True)
        self.optimizer_KGIN.step()

    def _train_epoch(self):
        self.KGIN_model.train()
        rec_all_loss_list = []
        auc_list = []
        ...
        for data in self.train_dataloader:
            ...
            loss, scores, rec_loss, emb_loss = self.KGIN_model(user_index, candidate_newsindex, label)
            self.optimize_MMCCLK(loss)
    ...
    def _vaild_epoch(self):
        ...
        self.KGIN_model.eval()
        rec_auc_list = []
        with no_grad():
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                loss, scores, rec_loss, emb_loss = self.KGIN_model(
                    user_index, candidate_newsindex, label
                )
                rec_auc = self.cal_auc(scores, label)
    ...
    def _save_checkpoint(self, epoch):
        state = self.KGIN_model.state_dict()
        filename = self.args.checkpoint_dir + ('checkpoint-KGIN-epoch{}.pth'.format(epoch))
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                scores = self.KGIN_model.test(user_index, candidate_newsindex)
        ...
        predict_df.to_csv(f'{folder_path}/KGIN_predict.csv', index = False)
