...
class Trainer():
    def __init__(self, args, KIM_model, optimizer_KIM, data):
        self.args = args
        self.KIM_model = KIM_model
        self.optimizer_KIM = optimizer_KIM
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _save_checkpoint(self, epoch):
        state_KIM_model = self.KIM_model.state_dict()
        filename_KIM = self.args.checkpoint_dir + ('checkpoint-KIM-epoch{}.pth'.format(epoch))
        torch.save(state_KIM_model, filename_KIM)
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                rec_score = self.KIM_model.test(candidate_newsindex, user_clicked_newsindex)
                score = rec_score
        ...
        predict_df.to_csv(f'{folder_path}/KIM_predict.csv', index = False)
        ...
        w_AUC, w_MRR, w_nDCG5, w_nDCG10, w_len, = evaluate_warm_cold_u(
            pred_label_list, user_type_list, news_type_list, self.label_test, self.bound_test
        )
        ...
        print('================news====================')
        c_AUC, c_MRR, c_nDCG5, c_nDCG10, c_len, \
        w_AUC, w_MRR, w_nDCG5, w_nDCG10, w_len = evaluate_warm_cold_n(
            pred_label_list, user_type_list, news_type_list, self.label_test, self.bound_test
        )
        print("c_AUC = %.4lf, c_MRR = %.4lf, c_nDCG5 = %.4lf, c_nDCG10 = %.4lf, c_len = %.4lf" %
              (c_AUC, c_MRR, c_nDCG5, c_nDCG10, c_len))
        ...
        ww_AUC, ww_MRR, ww_nDCG5, ww_nDCG10, ww_len = evaluate_warm_cold(
            pred_label_list, user_type_list, news_type_list, self.label_test, self.bound_test
        )
