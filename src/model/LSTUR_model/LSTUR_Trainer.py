...
class Trainer():
    def __init__(self, args, LSTUR_model, optimizer_LSTUR, data):
        self.args = args
        self.LSTUR_model = LSTUR_model
        self.optimizer_LSTUR = optimizer_LSTUR
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.LSTUR_model(user_index, candidate_newsindex, user_clicked_newsindex)
    ...
    def _vaild_epoch(self):
        self.LSTUR_model.eval()
        rec_auc_list = []
        with no_grad():
            pbar = tqdm(total=self.vailddata_size)
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.LSTUR_model.test(user_index, candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_LSTUR_model = self.LSTUR_model.state_dict()
        filename_LSTUR = self.args.checkpoint_dir + ('checkpoint-LSTUR-epoch{}.pth'.format(epoch))
        torch.save(state_LSTUR_model, filename_LSTUR)
    ...
    def test(self):
        ...
        predict_df.to_csv(f'{folder_path}/LSTUR_predict.csv', index = False)
