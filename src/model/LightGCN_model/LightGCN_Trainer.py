...
class Trainer():
    def __init__(self, args, LightGCN_model, optimizer_LightGCN, data):
        self.args = args
        self.LightGCN_model = LightGCN_model
        self.optimizer_LightGCN = optimizer_LightGCN
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            loss, rec_score, _, _ = self.LightGCN_model(user_index, candidate_newsindex, label)
            rec_loss = self.optimize_model(loss)
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.LightGCN_model.test(user_index, candidate_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_LightGCN_model = self.LightGCN_model.state_dict()
        filename_LightGCN = self.args.checkpoint_dir + ('checkpoint-LightGCN-epoch{}.pth'.format(epoch))
        torch.save(state_LightGCN_model, filename_LightGCN)
    ...
    def test(self):
        ...
        predict_df.to_csv(f'{folder_path}/LightGCN_predict.csv', index = False)
