from model.MCCM_model.MCCM import MCCM
from model.MCCM_model.MCCM_Trainer import Trainer
import torch

class MCCM_Train_Test():
    def __init__(self, args, data, device):
        ...
        MCCM_model = MCCM(args, entity_embedding, relation_embedding,
        ...
        optimizer_MCCM = torch.optim.Adam(MCCM_model.parameters(), lr=0.0001)
        self.trainer = Trainer(args, MCCM_model, optimizer_MCCM, data)
    ...
    def Test_load(self):
        self.MCCM_model.load_state_dict(torch.load(self.args.checkpoint_dir + 'checkpoint-' + self.args.mode + '-epochfinal.pth'))
        self.trainer.test()