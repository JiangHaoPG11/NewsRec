...
# import utils.tsneutil as tsneutil
# from openTSNE import TSNE

from sklearn.manifold import TSNE

class Trainer():
    def __init__(self, args, MCCM_model, optimizer_MCCM, data):
        self.args = args
        self.MCCM_model = MCCM_model
        self.optimizer_MCCM = optimizer_MCCM
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score, contrast_loss = self.MCCM_model(candidate_newsindex, user_clicked_newsindex)
            rec_loss = self.optimize_model(rec_score, contrast_loss, label.to(self.device))
            ...
            contrast_all_loss_list.append(contrast_loss.cpu().item())
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.MCCM_model.test(candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_MCCM_model = self.MCCM_model.state_dict()
        filename_MCCM = self.args.checkpoint_dir + ('checkpoint-MCCM-epoch{}.pth'.format(epoch))
        torch.save(state_MCCM_model, filename_MCCM)
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                rec_score, user_rep, news_rep = self.MCCM_model.test(candidate_newsindex, user_clicked_newsindex)
                score = rec_score
        ...
        # np.save('user_type.npy', user_type_list1)
        ...
        # x_dr = pca.transform(np.array(user_rep_list))
        # np.save('MCCM_user_rep.npy', np.array(user_rep_list))
        ...
        # np.save('news_type.npy', news_type_list1)
        # # norm
        ...
        # np.save('MCCM_news_rep.npy', np.array(news_rep_list))
        ...
        # plt.legend()
        # plt.savefig("MCCM_news_rep.png")
        # plt.show()
        # plt.close()
        
        # 存储预测结果
        folder_path = '../predict/MCCM/'
        ...
        predict_df.to_csv(f'{folder_path}/MCCM_predict.csv', index = False)
