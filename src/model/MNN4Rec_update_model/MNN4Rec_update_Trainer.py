...
class Trainer():
    def __init__(self, args, MNN4Rec_update_model, optimizer_MNN4Rec_update, data):
        self.args = args
        self.MNN4Rec_update_model = MNN4Rec_update_model
        self.optimizer_MNN4Rec_update = optimizer_MNN4Rec_update
        self.save_period = 1
        self.vaild_period = 1
    ...
    def optimize_model(self, rec_score, label):
        ...
        self.optimizer_MNN4Rec_update.zero_grad()
        rec_loss.backward()
        #for name, param in self.model_recommender.named_parameters():
        #    print('%14s : %s' % (name, param.grad))
        self.optimizer_MNN4Rec_update.step()
        return rec_loss

    def _train_epoch(self, epoch):
        self.MNN4Rec_update_model.train()
        rec_all_loss_list = []
        auc_list = []
        rec_all_loss = 0
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.MNN4Rec_update_model(user_index, candidate_newsindex, user_clicked_newsindex)
    ...
    def _vaild_epoch(self):
        self.MNN4Rec_update_model.eval()
        rec_auc_list = []
        with no_grad():
            pbar = tqdm(total=self.vailddata_size)
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.MNN4Rec_update_model.test(user_index, candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_MNN4Rec_update_model = self.MNN4Rec_update_model.state_dict()
        filename_MNN4Rec_update = self.args.checkpoint_dir + ('checkpoint-MNN4Rec_update-epoch{}.pth'.format(epoch))
        torch.save(state_MNN4Rec_update_model, filename_MNN4Rec_update)
    ...
    def test(self):
        ...
        predict_df.to_csv(f'{folder_path}/MNN4Rec_update_' + str(self.args.use_news_relation) + '_predict.csv', index = False)
