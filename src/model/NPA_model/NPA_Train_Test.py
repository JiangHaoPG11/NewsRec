from model.NPA_model.NPA import NPA
from model.NPA_model.NPA_Trainer import Trainer
import torch

class NPA_Train_Test():
    def __init__(self, args, data, device):
        ...
        NPA_model = NPA(args, entity_embedding, relation_embedding,
        ...
        optimizer_NPA = torch.optim.Adam(NPA_model.parameters(), lr=2e-5)
        self.trainer = Trainer(args, NPA_model, optimizer_NPA, data)
    ...
    def Test_load(self):
        self.NPA_model.load_state_dict(torch.load(self.args.checkpoint_dir + 'checkpoint-' + self.args.mode + '-epochfinal.pth'))
        self.trainer.test()
