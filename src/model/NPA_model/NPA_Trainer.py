...
class Trainer():
    def __init__(self, args, NPA_model, optimizer_NPA, data):
        self.args = args
        self.NPA_model = NPA_model
        self.optimizer_NPA = optimizer_NPA
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.NPA_model(user_index, candidate_newsindex, user_clicked_newsindex)
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.NPA_model.test(user_index, candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_NPA_model = self.NPA_model.state_dict()
        filename_NPA = self.args.checkpoint_dir + ('checkpoint-NPA-epoch{}.pth'.format(epoch))
        torch.save(state_NPA_model, filename_NPA)
    ...
    def test(self):
        ...
        predict_df.to_csv('NPA_predict.csv', index = False)
