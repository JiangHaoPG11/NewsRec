...
class Net(torch.nn.Module):
    ...
    def forward(self, state_input, action_input):
        ...
        actor_x = torch.tanh(
            self.actor_l1(
                torch.cat(
                    [state_input, action_input], dim=-1)
                )
            )
        actor_out = torch.tanh(
            self.actor_l2(actor_x)
        )
        act_probs = self.sigmoid(
            self.actor_l3(actor_out)
        )
        # Critic
        critic_x = torch.tanh(
            self.critic_l1(
...
class RCENR(torch.nn.Module):
    ...
    def __init__(self, args, news_entity_dict, entity_news_dict, user_click_dict,
        ...
        self.type_embedding = nn.Embedding(5, self.args.embedding_dim)
