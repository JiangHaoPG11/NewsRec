...
class Trainer():
    ...
    def get_batch_reward(self, step_reward):
        ...
        if len(self.args.depth) == 3:
            # [bz, d(1-hop) * d(2-hop)]
            batch_rewards[1] = torch.reshape(
                batch_rewards[1], 
                (
                    batch_rewards[1].shape[0], 
                    self.args.depth[0], 
                    self.args.depth[1]
                )
            )  # bz, d(1-hop) , d(2-hop)
            ...
            batch_rewards[2] = torch.reshape(
                batch_rewards[2], 
                (
                    batch_rewards[2].shape[0], 
                    self.args.depth[0], 
                    self.args.depth[1], 
                    self.args.depth[2]
                )
        ...
        if len(self.args.depth) == 3:
            batch_rewards[1] = torch.reshape(
                ...
                (
                    batch_rewards[1].shape[0], 
                    self.args.depth[0] * self.args.depth[1]
                )
            )
            ...
            batch_rewards[2] = torch.reshape(
                ...
                (
                    batch_rewards[2].shape[0], 
                    self.args.depth[0] * self.args.depth[1] * self.args.depth[2]
                )
            )
    ...
    def optimize_subgraph(self,
        ...
        for i in range(3):
            ...
            critic_loss1, actor_loss1 = self.subgraph_model.step_update(
                act_probs_step1, q_values_step1,
                batch_reward1,
                1.6 - rec_loss.detach(),
            ...
            news_actor_loss.append(actor_loss1.mean())
            news_critic_loss.append(critic_loss1.mean())
        ...
        for i in range(3):
            ...
            critic_loss2, actor_loss2 = self.subgraph_model.step_update(
                act_probs_step2, q_values_step2,
                batch_reward2,
                1.6 - rec_loss.detach(),
                1.6 - reasoning_loss.detach(),
                0.5,
            ...
            user_actor_loss.append(actor_loss2.mean())
            user_critic_loss.append(critic_loss2.mean())
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            # construct subgraph
            news_act_probs_steps, news_q_values_steps, news_step_rewards, \
            news_graph, news_graph_relation, news_graph_type, \
            user_act_probs_steps, user_q_values_steps, user_step_rewards, \
            user_graph, user_graph_relation, user_graph_type = self.subgraph_model(
                user_index, user_clicked_newsindex, candidate_newsindex
            )  # [[bz, d(1-hop) * 1], [bz, d(1-hop) * d(2-hop)], [bz, d(1-hop) * d(2-hop) * d(3-hop)]]

            # reasoning
            rea_score, overlap_score, path_num, _, overlap_num = self.model_reasoner(
                candidate_newsindex, user_index, 
                news_graph, user_graph,
                news_graph_relation, user_graph_relation
            )[:5]
            rea_loss, rea_loss_mean, overlap_loss = self.optimize_reasoner(
                rea_score, overlap_score, label.to(self.device)
            )
            
            # recommendation
            rec_score = self.model_recommender(
                candidate_newsindex, user_clicked_newsindex, news_graph, user_graph
            )
            rec_loss, rec_loss_mean = self.optimize_recommender(rec_score, label.to(self.device))
            ...
            # opt subgraph network
            subgraph_loss = self.optimize_subgraph(
                news_batch_rewards, news_q_values_steps, news_act_probs_steps,
            ...
            rec_all_loss_list.append(rec_loss_mean.cpu().item())
            rea_all_loss_list.append(rea_loss_mean.cpu().item())
            overlap_all_loss_list.append(torch.mean(overlap_loss).cpu().item())
            auc_list.append(rec_auc)

            # print("subgraph loss:{}---recommend loss:{}---reason loss:{}-------path num:{} ---rec auc:{} ".format(
            #     str(subgraph_loss.cpu().item()),  
            #     str(torch.mean(rec_loss).cpu().item()), 
            #     str(torch.mean(rea_loss).cpu().item()), 
            #     str(path_num), 
            #     str(rec_auc)
            #     )
            # )
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            for data in self.vaild_dataloader:
                ...
                rec_score = self.model_recommender(
                    candidate_newsindex, user_clicked_newsindex, 
                    news_graph, user_graph
                )
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_subgraph = self.args.checkpoint_dir + ('checkpoint-RCENR-subgraph-epoch{}.pth'.format(epoch))
        torch.save(state_subgraph_model, filename_subgraph)
        filename_recommender = self.args.checkpoint_dir + ('checkpoint-RCENR-recommender-epoch{}.pth'.format(epoch))
        torch.save(state_recommender, filename_recommender)
        filename_reasoner = self.args.checkpoint_dir + ('checkpoint-RCENR-reasoner-epoch{}.pth'.format(epoch))
    ...
    def train(self):
        for epoch in range(1, self.args.epoch+1):
            ...
            print("epoch:{}---subgraph loss:{}---recommend loss:{}---reason loss:{}-----path num:{} ---nodes num:{}--rec auc:{} ".format(
                    epoch, 
                    str(subgraph_loss),  
                    str(rec_loss), 
                    str(rea_loss), 
            ...
            if predict_graph:
                ...
                for data in self.train_dataloader:
                    ...
                    news_graph_nodes.extend(
                        self.subgraph_model.get_subgraph_list(
                            self.subgraph_model(
                                user_index, user_clicked_newsindex, candidate_newsindex
                            )[3],
                    ...
                    if not os.path.exists("./RCENR_out"):
                        os.makedirs("./RCENR_out")

                    news_graph_file = open("./RCENR_out/news_file_" + str(epoch) + ".tsv", 'w', encoding='utf-8')
                    for i in range(self.args.batch_size):
                        news_graph_file.write(candidate_newsindex[i] + '\t' + ' '.join(list(set(news_graph_nodes[i]))) + '\n')
                    user_graph_file = open("./RCENR_out/user_file_" + str(epoch) + ".tsv", 'w', encoding='utf-8')
                    for i in range(self.args.batch_size):
                        user_graph_file.write(user_clicked_newsindex['item1'][i] + '\t' + ' '.join(list(user_graph_nodes[i])) + '\n')
    ...
    def test(self):
        ...
        with no_grad():
            for data in self.test_dataloader:
                ...
                rea_score, _, path_num, path_num_list, \
                total_overlap_num, overlap_entity_num_list, \
                reasoning_paths, reasoning_edges, \
                news_graph_list, user_graph_list, news_graph_nodes_num, user_graph_nodes_num= self.model_reasoner(
                    candidate_newsindex, user_index, 
                    news_graph, user_graph, 
                    news_graph_relation, user_graph_relation
                )
        ...
        predict_df.to_csv(f'{folder_path}/RCENR_predict.csv', index = False)
        ...
        print('================news====================')
        c_AUC, c_MRR, c_nDCG5, c_nDCG10, c_len, \
        w_AUC, w_MRR, w_nDCG5, w_nDCG10, w_len = evaluate_warm_cold_n(pred_label_list, 
