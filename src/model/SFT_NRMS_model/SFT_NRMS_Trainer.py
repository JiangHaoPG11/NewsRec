...
class Trainer():
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_Contrastive_NRMS_ATT = self.args.checkpoint_dir + ('checkpoint-SFT_NRMS-epoch{}.pth'.format(epoch))
        torch.save(state_model, filename_Contrastive_NRMS_ATT)
    ...
    def test(self):
        ...
        np.save('Zeroshot_NRMS_user_rep.npy', np.array(user_rep_list))
        ...
        plt.legend()
        plt.savefig("Zeroshot_NRMS_user_rep.png")
        plt.show()
        plt.close()
        ...
        np.save('Zeroshot_NRMS_news_rep.npy', np.array(news_rep_list))
        ...
        plt.legend()
        plt.savefig("Zeroshot_NRMS_news_rep.png")
        plt.show()
        plt.close()

        # 存储预测结果
        folder_path = '../predict/Contrastive_NRMS_ATT/'
        ...
        predict_df.to_csv(f'{folder_path}/SFT_NRMS_predict.csv', index = False)
